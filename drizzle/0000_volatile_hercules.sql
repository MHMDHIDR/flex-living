...
CREATE TABLE "flex-living_post" (
	"id" integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY (sequence name "flex-living_post_id_seq" INCREMENT BY 1 MINVALUE 1 MAXVALUE 2147483647 START WITH 1 CACHE 1),
	"name" varchar(256),
...
ALTER TABLE "flex-living_post" ADD CONSTRAINT "flex-living_post_createdById_flex-living_user_id_fk" FOREIGN KEY ("createdById") REFERENCES "public"."flex-living_user"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
CREATE INDEX "created_by_idx" ON "flex-living_post" USING btree ("createdById");--> statement-breakpoint
CREATE INDEX "name_idx" ON "flex-living_post" USING btree ("name");--> statement-breakpoint
CREATE INDEX "t_user_id_idx" ON "flex-living_session" USING btree ("userId");