...
>(data: T[], columns: ColumnDef<T>[], filename = "export"): void {
  ...
  const exportableColumns = columns.filter((col) => {
    ...
    return (
      accessorKey &&
      !skipColumns.includes(accessorKey) &&
      !skipColumns.includes(id)
    );
  });
  ...
  const csvRows = data.map((row) => {
    return exportableColumns.map((col) => {
      ...
      if (
        value &&
        typeof value === "object" &&
        (value as unknown as Date) instanceof Date
      ) {
...
export function useCSVExport<
  T extends Record<string, string | number | Date | null | undefined>,
>(data: T[], columns: ColumnDef<T>[], defaultFilename = "export") {
...
export function exportReviewsToCSV(
  ...
  const exportData = reviews.map((item) => ({
    ...
    status: item.review.isApproved ? "Approved" : "Pending",
    submitted_date: new Date(item.review.submittedAt).toLocaleDateString(),
    external_id: item.review.externalId,
  }));

  // Use a simple CSV export for reviews
